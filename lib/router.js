...
import '/imports/ui/pages/Settings/A_propos/A_propos.html';
...
FlowRouter.route('/new/account', {
    name: 'New.account',
    action: function() {
        BlazeLayout.render('App', {main: 'Profile_creation'});
    }
});
...
FlowRouter.route('/àpropos', {
    name: 'A_propos.call',
    action: function() {
        BlazeLayout.render('App', {main: 'A_propos'});
    }
});

// SPECIAL ROUTES

// when the URL is totally wrong (matches no routed URL), no matter
// the params given (e.g. `/abcdefg`)
FlowRouter.notFound = {
    action() {
